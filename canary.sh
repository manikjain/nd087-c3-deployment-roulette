...
function canary_deploy {
  ...
  COUNT_V2=$((NUM_OF_V2_PODS + DEPLOY_INCREMENTS))
  COUNT_V1=$((NUM_OF_V1_PODS - DEPLOY_INCREMENTS))

  kubectl scale deployment canary-v2 --replicas=$COUNT_V2
  kubectl scale deployment canary-v1 --replicas=$COUNT_V1
